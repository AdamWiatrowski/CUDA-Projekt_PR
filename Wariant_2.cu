...
#define K 2 // ilosc elementow obliczana przez kazdy watek

#define OUT_SIZE (N - 2 * R) // rozmiar tablicy wyjsciowej
#define BLOCK_SIZE 32 // rozmiar bloku
#define GRID_SIZE ((OUT_SIZE/BLOCK_SIZE)*(OUT_SIZE/(K*BLOCK_SIZE))) // rozmiar gridu
#define SIZE_X (BLOCK_SIZE + 2*R) //size pobranych danych
#define SIZE_Y (K*BLOCK_SIZE + 2*R) //size pobranych danych


__global__ void calculate(int* out, int* tab, int N_k, int R_k, int K_k) {

    __shared__ int shared_tab[SIZE_X][SIZE_X];
    ...
    // dla kazdego k pobierane sa dane, inaczej by sie nie zmiescilo do pamieci wspoldzielonej
    for (int offset = 0; offset < K_k; offset++) {
        if (threadIdx.x == 0 && threadIdx.y == 0) {
            for (int row = 0; row < SIZE_X; row++) {
                for (int col = 0; col < SIZE_X; col++) {
                    shared_tab[row][col] = tab[(row + shift_row) * N_k + col + shift_col + (offset * BLOCK_SIZE)];
                }
            }
        }
        ...
        out[(row + shift_row) * OUT_SIZE + (col + shift_col + (offset * BLOCK_SIZE))] = sum;
